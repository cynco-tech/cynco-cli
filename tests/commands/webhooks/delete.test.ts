...
describe('webhooks delete command', () => {
	...
	test('deletes webhook with --yes', async () => {
		const spies = setupOutputSpies();
		const { deleteWebhookCmd } = await import('../../../src/commands/webhooks/delete');
		await deleteWebhookCmd.parseAsync(['wh_abc123', '--yes'], { from: 'user' });
	...
	test('errors without --yes in non-interactive mode', async () => {
		...
		mockExitThrow();
		const { deleteWebhookCmd } = await import('../../../src/commands/webhooks/delete');
		await expect(deleteWebhookCmd.parseAsync(['wh_abc123'], { from: 'user' })).rejects.toThrow(
			ExitError,
		);
	});
});
