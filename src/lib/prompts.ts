...
export async function confirmDelete(confirmMessage: string, globalOpts: GlobalOpts): Promise<void> {
	if (!isInteractive()) {
		outputError(
			{
...
export async function promptRenameIfInvalid(
	...
	p.log.warn(`Profile "${profileName}" has an invalid name: ${validationError}`);
...
export async function requireText(
	...
	const result = await p.text({
		...
		validate:
			prompt.validate ??
			((v) => (!v || v.length === 0 ? `${prompt.message} is required` : undefined)),
	});
			...
			missing.map((field) => [
				...
				() =>
					p.text({
						...
						validate:
							field.validate ??
							((v) => (!v || v.length === 0 ? `${field.message} is required` : undefined)),
					}),
			]),
		),
		{
